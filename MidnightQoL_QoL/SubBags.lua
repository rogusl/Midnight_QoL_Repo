    ...
    for _,bagID in ipairs(GetAllBagIDs()) do
        ...
        for slotID=1,n do
            ...
            if info and info.hyperlink then
                ...
                -- Determine bound status using the item instance's actual bind type.
                -- C_Item.GetItemBindType(location) returns:
                --   0 = unbound, 1 = BoE, 2 = BoU, 3 = Soulbound, 4 = Warbound
                -- This is the most reliable method — it reflects the actual state
                -- of the specific item in the bag, not just what the item type allows.
                local isBound   = false
                local isWarbound = false
                if loc and C_Item.DoesItemExist(loc) and C_Item.GetItemBindType then
                    local bindType = C_Item.GetItemBindType(loc)
                    if bindType == 3 then
                        isBound = true
                    ...
                    isWarbound = info.isWarboundByAcquisition or false
                end
...
local function BuildBagsTab()
    ...
    API.RegisterTab("Bags",bagsTabFrame,SyncBagsTab,80,nil,55)
end
