...
-- Position: draggable in Layout Mode; saved to BuffAlertDB.preyBarX/Y.
-- ============================================================
...
local function GetDB()
    ...
    if BuffAlertDB.preyBarEnabled == nil then BuffAlertDB.preyBarEnabled = false end
    return BuffAlertDB
end
...
local preyBar = CreateFrame("Frame", "MidnightQoLPreyBar", UIParent)
...
preyBar:SetScript("OnDragStop", function(self)
    ...
    if BuffAlertDB then
        BuffAlertDB.preyBarX = math.floor(self:GetLeft() + self:GetWidth()/2  - UIParent:GetWidth()/2  + 0.5)
        BuffAlertDB.preyBarY = math.floor(self:GetTop()  - self:GetHeight()/2 - UIParent:GetHeight()/2 + 0.5)
    end
end)
preyBar:Hide()
...
preyBorder:SetBackdrop({ edgeFile = "Interface/Buttons/WHITE8x8", edgeSize = 1 })
...
API.RegisterLayoutHandles(function()
    ...
    return {{
        ...
        saveCallback = function(nx, ny)
            if BuffAlertDB then BuffAlertDB.preyBarX = nx; BuffAlertDB.preyBarY = ny end
            preyBar:ClearAllPoints()
        ...
        resizeCallback = function(nw, nh)
            ...
            if BuffAlertDB then BuffAlertDB.preyBarW = nw; BuffAlertDB.preyBarH = nh end
        end,
    }}
end)

API.Debug("[MidnightQoL] PreyBar loaded.")
