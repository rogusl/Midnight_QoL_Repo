...
local function GetDB()
    ...
    if RogUIDB.poisonAlertEnabled   == nil then RogUIDB.poisonAlertEnabled   = false end
    if RogUIDB.raidbuffCheckEnabled == nil then RogUIDB.raidbuffCheckEnabled = false end
    if RogUIDB.battlerezEnabled     == nil then RogUIDB.battlerezEnabled     = false end
    return RogUIDB
end
...
local function CheckRoguePoisons()
    ...
    if #missing > 0 then
        print("|cFFFF0000[RogUI]|r |cFFFFD700Rogue: Missing " .. table.concat(missing, " and ") .. "!|r")
    end
end
...
-- HUD frame
local raidBuffHUD = CreateFrame("Frame", "RogUIRaidBuffHUD", UIParent, "BackdropTemplate")
...
raidBuffHUD:SetScript("OnDragStop",  function(self) self:StopMovingOrSizing()
    if RogUIDB then RogUIDB.raidBuffHUDX = self:GetLeft(); RogUIDB.raidBuffHUDY = self:GetTop() end
end)
raidBuffHUD:SetBackdropTexture("Interface/Buttons/WHITE8x8")
    raidBuffHUD:SetBackdropBorderTexture("Interface/DialogFrame/UI-DialogBox-Border")
    raidBuffHUD:SetBackdropBorderSizeZ(10)
    raidBuffHUD:SetBackdropInsets(4, 4, 4, 4)
...
local function ShowRaidBuffHUD(missing)
    ...
    if RogUIDB and RogUIDB.raidBuffHUDX and RogUIDB.raidBuffHUDY then
        raidBuffHUD:ClearAllPoints()
        raidBuffHUD:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT",
            RogUIDB.raidBuffHUDX, RogUIDB.raidBuffHUDY)
    end
...
local brezFrame = CreateFrame("Frame", "RogUIBrezFrame", UIParent,
...
brezFrame:SetScript("OnDragStop", function(self)
    ...
    if RogUIDB then RogUIDB.brezX = self:GetLeft(); RogUIDB.brezY = self:GetTop() end
end)
brezFrame:Hide()
...
brezBorder:SetBackdropBorderTexture("Interface/Buttons/WHITE8x8")
    brezBorder:SetBackdropBorderSizeZ(1)
...
local function UpdateBrezVisibility()
    ...
    if instanceType == "raid" or isMythicPlus or instanceType == "scenario" then
        ...
        if RogUIDB and RogUIDB.brezX and RogUIDB.brezY then
            pcall(function()
                brezFrame:ClearAllPoints()
                brezFrame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT",
                    RogUIDB.brezX, RogUIDB.brezY)
            end)
        end
        brezFrame:Show()
    else
...
utilEvents:SetScript("OnEvent", function(self, event, unit, ...)
    ...
    if event == "PLAYER_LOGIN" then
        C_Timer.After(2, function()
            ...
            local lib = GetOpenRaidLib()
            if lib then
                local RogUICallbacks = {}
                function RogUICallbacks.OnUnitInfoUpdate()
                    CheckRaidBuffs()
                end
                lib.RegisterCallback(RogUICallbacks, "UnitInfoUpdate", "OnUnitInfoUpdate")
                lib.RequestAllData()
            end
        end)
...
API.RegisterLayoutHandles(function()
    ...
    do
        ...
        pcall(function()
            brezFrame:ClearAllPoints()
            local bx = (RogUIDB and RogUIDB.brezX) or (sw/2 - brezFrame:GetWidth()/2)
            local by = (RogUIDB and RogUIDB.brezY) or (sh/2 - 160)
        ...
        table.insert(handles, {
            ...
            saveCallback = function(nx, ny)
                ...
                if RogUIDB then RogUIDB.brezX = nbx; RogUIDB.brezY = nby end
            end,
        })
    end
    ...
    do
        ...
        local hx = (RogUIDB and RogUIDB.raidBuffHUDX) or (sw/2 - 60)
        local hy = (RogUIDB and RogUIDB.raidBuffHUDY) or (sh/2 + 200)
        raidBuffHUD:ClearAllPoints()
        ...
        table.insert(handles, {
            ...
            saveCallback = function(nx, ny)
                ...
                if RogUIDB then RogUIDB.raidBuffHUDX = nbx; RogUIDB.raidBuffHUDY = nby end
            end,
        })
    end
...
API.Debug("[RogUI] Utility module loaded.")
