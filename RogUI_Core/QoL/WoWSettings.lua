...
local function GetDB()
    ...
    RogUIDB.wowSettings = RogUIDB.wowSettings or {}
    return RogUIDB.wowSettings
end
...
local function ApplyEditModeLayout(silent)
    ...
    if not silent then
        print("|cFF00CCFF[RogUI]|r Edit Mode layout applied.")
    end
end
...
local function CaptureSettings()
    local db = GetDB()
    if not db then
        print("|cFFFF4444[RogUI]|r Defaults: database not ready.")
        return
    end
    ...
    print("|cFF00CCFF[RogUI]|r WoW settings snapshot saved  |cFFAAAAAA(" .. db.savedAt .. ")|r" .. editMsg)
end
...
local function ApplySettings(silent)
    ...
    if CompactRaidFrameManager_UpdateShown then
        ...
        if InCombatLockdown() and not _G.RogUI_RaidFramePendingUpdate then
            _G.RogUI_RaidFramePendingUpdate = true
            local f = CreateFrame("Frame")
            f:RegisterEvent("PLAYER_REGEN_ENABLED")
            f:SetScript("OnEvent", function(self)
                self:UnregisterAllEvents()
                _G.RogUI_RaidFramePendingUpdate = nil
                C_Timer.After(0, CompactRaidFrameManager_UpdateShown)
            end)
        end
    end
    ...
    if not silent then
        print(string.format("|cFF00CCFF[RogUI]|r Applied %d saved WoW settings.", count))
    end
end
...
local function BuildUI()
    ...
    local f = CreateFrame("Frame", "RogUIWoWSettingsFrame", UIParent)
    ...
    -- How to use
    y = y - 8
    Note("|cFFFFD700How to use:|r  Set up your UI exactly how you want it. Click Save. On every subsequent login RogUI silently re-applies your snapshot — including on alts. Apply Now forces an immediate re-apply.", 12)
...
API.Debug("[RogUI] WoWSettings loaded")
