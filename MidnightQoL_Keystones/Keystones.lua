...
--   5. Tooltip %       — M+ count % injected into enemy tooltips (requires MDT)
--   6. Settings tab    — registered into MidnightQoL tab system
-- ============================================================
...
local DEFAULTS = {
    ...
    announceWinner = true,
    wheelSpinTime  = 5,
    tooltipPercent = true,   -- show M+ count % in enemy tooltips
}
...
-- ── Settings tab ──────────────────────────────────────────────────────────────
-- ── M+ count % tooltip hook ──────────────────────────────────────────────────────────────────────────────────
-- Injects "<count> (<pct>%)" into enemy tooltips during an active M+ run.
-- Requires MDT (MythicDungeonTools) for per-NPC weight data.
local function GetMDTWeight(npcID)
    local DT = _G.MDT or _G.MethodDungeonTools
    if not DT or not DT.GetEnemyForces then return nil, nil end
    local count, max = DT:GetEnemyForces(tonumber(npcID))
    if not count or not max or max == 0 then return nil, nil end
    return count, max
end

local function HookTooltipPercent()
    local function OnUnit(tooltip)
        if not GetDB().tooltipPercent then return end
        if not C_ChallengeMode.IsChallengeModeActive() then return end

        local _, unit = tooltip:GetUnit()
        if not unit then return end
        if not UnitExists(unit) then return end
        -- Only hostile / neutral NPCs
        local reaction = UnitReaction(unit, "player")
        if reaction and reaction > 4 then return end
        ...
        local count, max = GetMDTWeight(npcID)
        if not count then return end

        local pct = (count / max) * 100
        tooltip:AddLine(string.format("|cFFFFD700M+ Count:|r  %d  |cFFAAAAAA(%.2f%%)|r", count, pct), 1, 1, 1)
        tooltip:Show()
    end
    ...
    if TooltipDataProcessor and TooltipDataProcessor.AddTooltipPostCall then
        ...
        -- Fallback
        GameTooltip:HookScript("OnTooltipSetUnit", function(self) OnUnit(self) end)
    end
end
...
local function BuildSettingsTab()
    ...
    Check("Show M+ count % in enemy tooltips (requires MDT)", "tooltipPercent")
    ...
    info:SetPoint("TOPLEFT",12,y); info:SetWidth(720); info:SetJustifyH("LEFT"); info:SetWordWrap(true)
    ...
    statusLbl:SetPoint("TOPLEFT",12,y); statusLbl:SetWidth(720)
...
ev:SetScript("OnEvent",function(self,event,...)
    if event=="PLAYER_LOGIN" then
        ...
        if not API.IsTabEnabled or API.IsTabEnabled("Keystones") then
            C_Timer.After(0, HookTooltipPercent)
        end
