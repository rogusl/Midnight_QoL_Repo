-- ============================================================
-- MidnightQoL / Modules / ImprovedCDM / ImprovedCDMUI.lua
...
do
    ...
    local dragGhost = CreateFrame("Frame", "MQoL_UI_DragGhost", UIParent)
    ...
    local function BuildAddIconPopup()
        ...
        local p = CreateFrame("Frame", "MQoLAddIconPopup", UIParent, "BackdropTemplate")
        p:SetSize(300, 200)
...
-- Settings stored in BuffAlertDB.iconSettings[spellID].
-- ════════════════════════════════════════════════════════════
do
    local popup = nil
    ...
    local function GetIconDB(spellID)
        ...
        BuffAlertDB.iconSettings = BuffAlertDB.iconSettings or {}
        BuffAlertDB.iconSettings[spellID] = BuffAlertDB.iconSettings[spellID] or {}
        return BuffAlertDB.iconSettings[spellID]
    end

    local function BuildPopup()
        if popup then return popup end

        local p = CreateFrame("Frame", "MQoLIconConfigModal", UIParent, "BackdropTemplate")
        p:SetSize(320, 310)
        p:SetFrameStrata("DIALOG")
        p:SetFrameLevel(300)
        p:SetBackdrop({
        ...
        local soundDropdown = API.CreateSoundSelectorButton(p, "MQoLIconCfg_SoundDD")
    ...
    function API.GetIconSettings(spellID)
        if not spellID or not BuffAlertDB then return {} end
        BuffAlertDB.iconSettings = BuffAlertDB.iconSettings or {}
        return BuffAlertDB.iconSettings[spellID] or {}
    end
end
...
do
    ...
    encScroll:SetPoint("BOTTOMRIGHT", cdmFrame, "BOTTOMRIGHT", -36, 8)
...
do
    ...
    local function BuildTauntSwapSettingsPopup()
        local f = CreateFrame("Frame", "MQoLTauntSwapSettings", UIParent, "BackdropTemplate")
        f:SetSize(350, 250)
    ...
    tabs[1].onActivate = origTab1Activate   
end
...
print("|cFF00FF00[MidnightQoL] Cooldown Management UI loaded|r")