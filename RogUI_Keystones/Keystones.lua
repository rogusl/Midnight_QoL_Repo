-- ============================================================
-- RogUI / Modules / Keystones / Keystones.lua
...
--   5. Settings tab    — registered into RogUI tab system
-- ============================================================
...
local function BuildChallengesFramePanels()
    ...
    pp:SetBackdropTexture("Interface\\DialogFrame\\UI-DialogBox-Background")
    pp:SetBackdropBorderTexture("Interface\\DialogFrame\\UI-DialogBox-Border")
    pp:SetBackdropBorderSizeZ(12)
    pp:SetBackdropInsets(3, 3, 3, 3)
    ...
    dp:SetBackdropTexture("Interface\\DialogFrame\\UI-DialogBox-Background")
    dp:SetBackdropBorderTexture("Interface\\DialogFrame\\UI-DialogBox-Border")
    dp:SetBackdropBorderSizeZ(12)
    dp:SetBackdropInsets(3, 3, 3, 3)
...
local function BuildWheelFrame()
    ...
    f:SetBackdropTexture("Interface\\DialogFrame\\UI-DialogBox-Background")
    f:SetBackdropBorderTexture("Interface\\DialogFrame\\UI-DialogBox-Border")
    f:SetBackdropBorderSizeZ(16)
    f:SetBackdropInsets(4, 4, 4, 4)
    ...
    announceBtn:SetScript("OnClick",function()
        ...
        else print("|cFF00CCFF[RogUI]|r "..msg) end
    end)
...
local function BuildSettingsTab()
    ...
    local mainWin = _G["RogUIMainFrame"]

    -- 1. Master Container
    local container = CreateFrame("Frame", "RogUIKeystonesContainer", mainWin)
    container:SetSize(620, 520)
    -- Increased offset to -80 to give main tabs plenty of room
    container:SetPoint("TOPLEFT", mainWin, "TOPLEFT", 10, -80) 
    container:Hide()

    -- 2. Sub-tab Buttons
    local subtabRow = CreateFrame("Frame", nil, container)
    subtabRow:SetSize(600, 40)
    subtabRow:SetPoint("TOPLEFT", container, "TOPLEFT", 15, 0)

    local keystoneBtn = CreateFrame("Button", nil, subtabRow, "GameMenuButtonTemplate")
    keystoneBtn:SetSize(150, 26)
    keystoneBtn:SetPoint("LEFT", subtabRow, "LEFT", 0, 0)
    keystoneBtn:SetText("Keystone Tools")

    local timerBtn = CreateFrame("Button", nil, subtabRow, "GameMenuButtonTemplate")
    timerBtn:SetSize(150, 26)
    timerBtn:SetPoint("LEFT", keystoneBtn, "RIGHT", 10, 0) -- 10px gap between buttons
    timerBtn:SetText("M+ Timer")

    -- 3. Content Area
    local ksContent = CreateFrame("Frame", nil, container)
    ksContent:SetSize(600, 450)
    -- Content starts at -50 to leave clear space below the buttons
    ksContent:SetPoint("TOPLEFT", container, "TOPLEFT", 0, -50)
    ksContent:Hide()

    -- --- LEGIBLE SPACING LOGIC ---
    local y = -10
    local leftMargin = 20 -- Consistent indentation

    local function Header(txt)
        local t = ksContent:CreateFontString(nil, "ARTWORK", "GameFontNormalLarge")
        t:SetText(txt)
        t:SetPoint("TOPLEFT", leftMargin, y)
        y = y - 28 -- Increased from 24
    end

    local function Divider()
        local d = ksContent:CreateTexture(nil, "ARTWORK")
        d:SetSize(560, 1)
        d:SetPoint("TOPLEFT", leftMargin, y)
        d:SetColorTexture(1, 1, 1, 0.15) -- Subtle white line
        y = y - 15 -- Increased from 8
    end

    local checkCount = 0
    local function Check(label, dbKey)
        checkCount = checkCount + 1
        local c = CreateFrame("CheckButton", "MQLKeysCheck"..checkCount, ksContent, "UICheckButtonTemplate")
        c:SetSize(26, 26)
        c:SetPoint("TOPLEFT", leftMargin, y)
        _G[c:GetName().."Text"]:SetFontObject("GameFontHighlight") -- Brighter text
        _G[c:GetName().."Text"]:SetText(label)
        c:SetChecked(GetDB()[dbKey])
        c:SetScript("OnClick", function(self) GetDB()[dbKey] = self:GetChecked() end)
        y = y - 32 -- Increased from 26
        return c
    end
    ...
    local function Slider(label, dbKey, lo, hi, step)
        ...
        local lbl = ksContent:CreateFontString(nil, "ARTWORK", "GameFontNormal")
        lbl:SetText(label)
        lbl:SetPoint("TOPLEFT", leftMargin, y)
        y = y - 22
        
        local s = CreateFrame("Slider", "MQLKeysSlider"..sliderCount, ksContent, "OptionsSliderTemplate")
        s:SetSize(240, 18)
        s:SetPoint("TOPLEFT", leftMargin + 10, y)
        s:SetMinMaxValues(lo, hi)
        s:SetValueStep(step)
        s:SetValue(GetDB()[dbKey] or lo)
        _G[s:GetName().."Low"]:SetText(lo)
        _G[s:GetName().."High"]:SetText(hi)
        _G[s:GetName().."Text"]:SetText((GetDB()[dbKey] or lo).."s")
        s:SetScript("OnValueChanged", function(self, v)
            v = math.floor(v + 0.5)
            GetDB()[dbKey] = v
            _G[self:GetName().."Text"]:SetText(v.."s")
        end)
        y = y - 40 -- Increased from 28
    end

    -- Populate Keystones Tab with spacing fixes
    Header("|cFFFFD700General Settings|r")
    Divider()
    Check("Auto-slot keystone when M+ UI opens", "autoSlot")

    y = y - 15 -- Extra padding between sections
    Header("|cFFFFD700Keystone Wheel|r")
    Divider()
    Check("Announce winner to party chat", "announceWinner")
    Slider("Spin duration", "wheelSpinTime", 2, 15, 1)

    local openBtn = CreateFrame("Button", nil, ksContent, "UIPanelButtonTemplate")
    openBtn:SetSize(180, 28)
    openBtn:SetPoint("TOPLEFT", leftMargin, y)
    openBtn:SetText("Open Keystone Wheel")
    openBtn:SetScript("OnClick", function() if OpenWheel then OpenWheel() end end)
    
    y = y - 55 -- Extra padding
    Header("|cFFAAAAAAData Sources & Plugins|r")
    Divider()
    
    local info = ksContent:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    info:SetPoint("TOPLEFT", leftMargin, y)
    info:SetWidth(540)
    info:SetJustifyH("LEFT")
    info:SetText("Party keystones are collected from (priority order):\n1. LibOpenRaid-1.0  |  2. AngryKeystones  |  3. AstralKeys")
    y = y - 45

    local statusLbl = ksContent:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
    statusLbl:SetPoint("TOPLEFT", leftMargin, y)
    statusLbl:SetWidth(560)
    statusLbl:SetJustifyH("LEFT")

    ksContent:SetScript("OnShow", function()
        local libOK = LibStub and LibStub("LibOpenRaid-1.0", true) ~= nil
        statusLbl:SetText(string.format(
            "Status: LibOpenRaid (%s) | AngryKeystones (%s) | AstralKeys (%s)",
            libOK and "|cFF00FF00OK|r" or "|cFFFF4444N/A|r",
            C_AddOns.IsAddOnLoaded("AngryKeystones") and "|cFF00FF00OK|r" or "|cFFAAAAAAN/A|r",
            C_AddOns.IsAddOnLoaded("AstralKeys") and "|cFF00FF00OK|r" or "|cFFAAAAAAN/A|r"
        ))
    end)

    -- 4. Timer UI Integration
    if API.MythicTimerUIFrame then
        API.MythicTimerUIFrame:SetParent(container)
        API.MythicTimerUIFrame:ClearAllPoints()
        API.MythicTimerUIFrame:SetPoint("TOPLEFT", container, "TOPLEFT", 15, -50)
        API.MythicTimerUIFrame:Hide()
    end

    -- 5. Navigation Logic
    local function ShowKeystones()
        ksContent:Show()
        if API.MythicTimerUIFrame then API.MythicTimerUIFrame:Hide() end
        keystoneBtn:SetButtonState("PUSHED", true)
        timerBtn:SetButtonState("NORMAL", false)
    end
    local function ShowTimer()
        ksContent:Hide()
        if API.MythicTimerUIFrame then API.MythicTimerUIFrame:Show() end
        keystoneBtn:SetButtonState("NORMAL", false)
        timerBtn:SetButtonState("PUSHED", true)
    end

    keystoneBtn:SetScript("OnClick", ShowKeystones)
    timerBtn:SetScript("OnClick", ShowTimer)

    API.RegisterTab("Keystones", container, function()
        if not ksContent:IsShown() and not (API.MythicTimerUIFrame and API.MythicTimerUIFrame:IsShown()) then
            ShowKeystones()
        end
    end, 72, nil, 5)
end
...
-- ev frame replaced by API.RegisterEvent calls

local function OnKeystonesEvent(event, ...)
...
API.RegisterEvent("Keystones", "PLAYER_LOGIN",              function(...) OnKeystonesEvent("PLAYER_LOGIN", ...) end)
API.RegisterEvent("Keystones", "ADDON_LOADED",              function(...) OnKeystonesEvent("ADDON_LOADED", ...) end)
API.RegisterEvent("Keystones", "CHAT_MSG_ADDON",            function(...) OnKeystonesEvent("CHAT_MSG_ADDON", ...) end)
API.RegisterEvent("Keystones", "GROUP_ROSTER_UPDATE",       function(...) OnKeystonesEvent("GROUP_ROSTER_UPDATE", ...) end)
API.RegisterEvent("Keystones", "BAG_UPDATE_DELAYED",        function(...) OnKeystonesEvent("BAG_UPDATE_DELAYED", ...) end)
API.RegisterEvent("Keystones", "CHALLENGE_MODE_START",      function(...) OnKeystonesEvent("CHALLENGE_MODE_START", ...) end)
API.RegisterEvent("Keystones", "CHALLENGE_MODE_COMPLETED",  function(...) OnKeystonesEvent("CHALLENGE_MODE_COMPLETED", ...) end)
API.RegisterEvent("Keystones", "CHALLENGE_MODE_MAPS_UPDATE",function(...) OnKeystonesEvent("CHALLENGE_MODE_MAPS_UPDATE", ...) end)
...
API.Debug("[RogUI Keystones] loaded  /mqlwheel to spin")
