-- ============================================================
-- RogUI / Modules / ResourceBars / ResourceBars.lua
...
local PIP_SHAPES = {
    ...
    -- ── Diamond ───────────────────────────────────────────────────────────────
    {
        key="diamond",
        label="Diamond",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_diamond",
    },
    -- ── Rune ─────────────────────────────────────────────────────────────────
    {
        key="rune",
        label="Rune",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_rune",
    },
    -- ── Star ─────────────────────────────────────────────────────────────────
    {
        key="star",
        label="Star",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_star",
    },
    -- ── Arrow ────────────────────────────────────────────────────────────────
    {
        key="arrow",
        label="Arrow",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_arrow",
    },
    -- ── Skull ────────────────────────────────────────────────────────────────
    {
        key="skull",
        label="Skull",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_skull",
    },
    -- ── Moon ─────────────────────────────────────────────────────────────────
    {
        key="moon",
        label="Moon",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_moon",
    },
    -- ── Lightning ─────────────────────────────────────────────────────────────
    -- Custom white-on-transparent bolt silhouette bundled with the addon.
    {
        key="lightning",
        label="Lightning",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_lightning",
    },
    -- ── Spearhead ─────────────────────────────────────────────────────────────
    {
        key="spearhead",
        label="Spearhead",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_spearhead",
    },
    -- ── Icicle ────────────────────────────────────────────────────────────────
    {
        key="icicle",
        label="Icicle",
        tex="Interface\\AddOns\\RogUI\\Images\\pip_icicle",
    },
}
...
local barFrames  = {}
local roleUnitCache = {}
local globalResourceBarAlpha = 1.0  -- NEW: Global alpha for all resource bars
...
-- NEW: Apply global alpha to resource bars
local function ApplyBarAlpha(barFrame)
    if barFrame then
        barFrame:SetAlpha(globalResourceBarAlpha)
    end
end
...
local function RebuildBars()
    for i = 1, MAX_BARS do
        ...
        if cfg and cfg.enabled and resourceBarsEnabled then
            ...
            f:Show()
            -- NEW: Apply alpha when bar is shown
            ApplyBarAlpha(f)
            RefreshBar(i)
        else
            f:Hide()
...
-- NEW: API function to apply alpha to all resource bars
function API.ResourceBars_ApplyAlpha(alpha)
    globalResourceBarAlpha = alpha or 1.0
    for i = 1, MAX_BARS do
        if barFrames[i] then
            ApplyBarAlpha(barFrames[i])
        end
    end
end
        ...
        if f and f:IsShown() and cfg and cfg.enabled then
            ...
            table.insert(handles, {
                ...
                saveCallback = function(nx, ny)
                    cfg.x = nx; cfg.y = ny
                    if RogUIDB and API.SaveSpecProfile then API.SaveSpecProfile() end
                end,
...
-- ── Event Registry (unified system) ─────────────────────────────────────────
local function OnResourceBarEvent(unit)
    if not unit then
        -- GROUP_ROSTER_UPDATE or PLAYER_ENTERING_WORLD (no unit arg)
        wipe(roleUnitCache)
...
API.RegisterEvent("ResourceBars", "PLAYER_ENTERING_WORLD", OnPlayerEnteringWorld)
API.RegisterEvent("ResourceBars", "UNIT_HEALTH",           OnResourceBarEvent)
API.RegisterEvent("ResourceBars", "UNIT_POWER_UPDATE",     OnResourceBarEvent)
API.RegisterEvent("ResourceBars", "UNIT_MAXPOWER",         OnResourceBarEvent)
API.RegisterEvent("ResourceBars", "UNIT_AURA",             OnResourceBarEvent)
API.RegisterEvent("ResourceBars", "GROUP_ROSTER_UPDATE",   function() OnResourceBarEvent(nil) end)
-- ACTIONBAR_SLOT_CHANGED registered but intentionally ignored (no-op handler)
...
API.RegisterProfileCallbacks(
    ...
    function(p)
        ...
        if RogUIDB then resourceBarsEnabled = RogUIDB.resourceBarsEnabled ~= false end
        RebuildBars()
    end
)