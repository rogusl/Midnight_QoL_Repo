-- ============================================================
-- RogUI_ResourceBars / ResourceBarsUI.lua
...
-- ── Content frame ─────────────────────────────────────────────────────────────
local contentFrame = CreateFrame("Frame","RogUIResourceBarsFrame",UIParent)
...
-- NEW: Resource Bars Opacity Slider
local resourceBarsAlphaSlider = CreateFrame("Slider", "ResourceBarsAlphaSlider", contentFrame, "OptionsSliderTemplate")
resourceBarsAlphaSlider:SetPoint("TOPRIGHT", contentFrame, "TOPRIGHT", -30, -10)
resourceBarsAlphaSlider:SetMinMaxValues(0.1, 1.0)
resourceBarsAlphaSlider:SetValueStep(0.05)
resourceBarsAlphaSlider:SetObeyStepOnDrag(true)
resourceBarsAlphaSlider:SetValue(1.0)  -- Start at full opacity
_G[resourceBarsAlphaSlider:GetName() .. "Low"]:SetText("10%")
_G[resourceBarsAlphaSlider:GetName() .. "High"]:SetText("100%")

local currentResourceAlpha = 1.0
_G[resourceBarsAlphaSlider:GetName() .. "Text"]:SetText("Resource Bars Opacity: " .. math.floor(currentResourceAlpha * 100) .. "%")

resourceBarsAlphaSlider:SetScript("OnValueChanged", function(self, value)
    value = math.floor(value * 20 + 0.5) / 20
    
    _G[self:GetName() .. "Text"]:SetText("Resource Bars Opacity: " .. math.floor(value * 100) .. "%")
    
    -- Apply to all resource bars
    local API = RogUIAPI
    if API and API.ResourceBars_ApplyAlpha then
        API.ResourceBars_ApplyAlpha(value)
    end
end)
