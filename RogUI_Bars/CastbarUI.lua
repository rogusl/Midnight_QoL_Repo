...
local function GetDB()
    if not RogUICastbarDB then RogUICastbarDB = {} end
    for k, v in pairs(DEFAULTS) do
        if RogUICastbarDB[k] == nil then RogUICastbarDB[k] = v end
    end
    return RogUICastbarDB
end
...
-- ── Content frame ─────────────────────────────────────────────────────────────
local cf = CreateFrame("Frame", "RogUICastbarFrame", UIParent)
cf:SetSize(620, 300)
cf:Hide()
...
-- NEW: Castbar Opacity Slider
local castbarAlphaSlider = CreateFrame("Slider", "CastbarAlphaSlider", cf, "OptionsSliderTemplate")
castbarAlphaSlider:SetPoint("TOPLEFT", cf, "TOPLEFT", 0, -50)
castbarAlphaSlider:SetMinMaxValues(0.1, 1.0)
castbarAlphaSlider:SetValueStep(0.05)
castbarAlphaSlider:SetObeyStepOnDrag(true)
castbarAlphaSlider:SetValue((GetDB() and GetDB().alpha) or 1.0)
_G[castbarAlphaSlider:GetName() .. "Low"]:SetText("10%")
_G[castbarAlphaSlider:GetName() .. "High"]:SetText("100%")

local currentCastbarAlpha = (GetDB() and GetDB().alpha) or 1.0
_G[castbarAlphaSlider:GetName() .. "Text"]:SetText("Castbar Opacity: " .. math.floor(currentCastbarAlpha * 100) .. "%")

castbarAlphaSlider:SetScript("OnValueChanged", function(self, value)
    ...
    _G[self:GetName() .. "Text"]:SetText("Castbar Opacity: " .. math.floor(value * 100) .. "%")
    
    -- Apply to castbar frame
    local API = RogUIAPI
    if API and API.Castbar_ApplyAlpha then
        API.Castbar_ApplyAlpha()
    end
end)
...
local function OnActivate()
    ...
    -- NEW: Set alpha slider
    castbarAlphaSlider:SetValue((db and db.alpha) or 1.0)
...
local function HarvestValues()
    ...
    -- NEW: Save alpha slider
    db.alpha         = castbarAlphaSlider:GetValue() or 1.0
...
if API.RegisterPreSaveCallback then
    ...
    frame:SetScript("OnEvent", function(self)
        local btn = _G["RogUISaveBtn"]
        if btn then btn:HookScript("OnClick", HarvestValues) end
        self:UnregisterAllEvents()
    end)
end

-- Flush text box values to RogUICastbarDB on logout so they survive without needing Save
