-- ============================================================
-- RogUI / Modules / Bags / SubBags.lua
...
-- Settings tab in RogUI panel → Bags
-- ============================================================
...
local function DB()
    ...
    if RogUIDB.subBags             == nil then RogUIDB.subBags             = {} end
    if RogUIDB.subBagIlvlEnabled   == nil then RogUIDB.subBagIlvlEnabled   = true end
    if RogUIDB.subBagIconSize      == nil then RogUIDB.subBagIconSize      = 37 end
    if RogUIDB.subBagCollapsed     == nil then RogUIDB.subBagCollapsed     = {} end
    if RogUIDB.subBagHideSoulbound == nil then RogUIDB.subBagHideSoulbound = false end
    if RogUIDB.subBagHideWarbound   == nil then RogUIDB.subBagHideWarbound   = false end
    if RogUIDB.subBagShowBoundOnly  == nil then RogUIDB.subBagShowBoundOnly  = false end
    if RogUIDB.bagUpgradeEnabled   == nil then RogUIDB.bagUpgradeEnabled   = true end
    return RogUIDB
end
    ...
    for _, setID in ipairs(C_EquipmentSet.GetEquipmentSetIDs()) do
        ...
        if setName then
            for _, locationID in pairs(C_EquipmentSet.GetItemLocations(setID) or {}) do
                if locationID and locationID ~= -1 then
...
-- ── Warbound detection via C_Item.GetItemBindType ────────────────────────────
-- GameTooltip:SetBagItem() is a protected call — invoking it from addon code
-- taints the current execution context and causes ADDON_ACTION_FORBIDDEN on any
-- subsequent protected call (e.g. UseContainerItem) in the same thread.
-- C_Item.GetItemBindType() is a pure data API and does not taint anything.
local WARBOUND_BIND = Enum.ItemBind and Enum.ItemBind.WarboundToAccount

local function IsWarboundItem(bagID, slotID)
    -- Fast path: use the bind-type API introduced in TWW/Midnight.
    if WARBOUND_BIND then
        local loc = ItemLocation:CreateFromBagAndSlot(bagID, slotID)
        if loc and C_Item.DoesItemExist(loc) then
            local ok, bindType = pcall(C_Item.GetItemBindType, loc)
            if ok and bindType == WARBOUND_BIND then return true end
        end
        return false
    end
    -- Fallback for older builds: check the item hyperlink flags.
    -- item:ITEMID:...:BINDTYPE:... — bindType 4 == Warband in older data.
    local info = C_Container.GetContainerItemInfo(bagID, slotID)
    if info and info.hyperlink then
        local bindType = tonumber(info.hyperlink:match("item:%d+:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*:(%d+)"))
        if bindType == 4 then return true end
    end
    return false
end
...
-- indexFrame pool — bag-ID carrier frames, pooled to avoid in-combat CreateFrame taint.
-- ContainerFrameItemButtonTemplate reads btn:GetParent():GetID() for the bagID, so each
-- button must be parented to one of these frames.  Creating them fresh during LayoutPanel
-- (which can fire in combat via BAG_UPDATE_DELAYED) taints the frame tree and causes
-- ADDON_ACTION_FORBIDDEN on UseContainerItem.  Pooling ensures they are created once,
-- out of combat, at addon initialisation time.
local indexFramePool   = {}
local indexFramePoolSz = 0

local function GetOrCreateIndexFrame()
    indexFramePoolSz = indexFramePoolSz + 1
    if indexFramePool[indexFramePoolSz] then return indexFramePool[indexFramePoolSz] end
    -- panel.canvas must exist before this is called (BuildPanel runs at PLAYER_LOGIN).
    local f = CreateFrame("Frame", nil, panel.canvas)
    indexFramePool[indexFramePoolSz] = f
    return f
end
...
local function GetOrCreateBtn()
    ...
    local btn = CreateFrame("ItemButton", nil, panel.canvas, "ContainerFrameItemButtonTemplate")

    -- Prevent the template's OnEvent glow triggers from firing.
    -- SetScript("OnEvent", nil) is forbidden on secure frames (ADDON_ACTION_FORBIDDEN).
    -- UnregisterAllEvents() is the correct taint-safe replacement — it simply
    -- removes all event listeners without touching the secure script environment.
    -- The OnShow SetScript is also forbidden; instead we stop the glow animation
    -- and hide the glow textures directly, which achieves the same result.
...
local function GetOrCreateHeader()
    ...
    -- Subtle background
    hdr:SetBackdrop({bgFile="Interface/Tooltips/UI-Tooltip-Background",edgeSize=0,insets={left=0,right=0,top=0,bottom=0}})
...
function LayoutPanel()
    ...
    for i=1,#indexFramePool do if indexFramePool[i] then indexFramePool[i]:Hide() end end
    btnPoolSz=0; hdrPoolSz=0; indexFramePoolSz=0
...
local function BuildPanel()
    ...
    panel=CreateFrame("Frame","RogUIBagPanel",UIParent,"BackdropTemplate")
    ...
    title:SetPoint("TOPLEFT",14,-10); title:SetText("|cFF00CCFFRogUI|r  Bags")

    local closeBtn=CreateFrame("Button",nil,panel,"UIPanelCloseButton")
    closeBtn:SetPoint("TOPRIGHT",-2,-2)
    closeBtn:SetScript("OnClick",function() panel:Hide() end)

    local searchBox=CreateFrame("EditBox","RogUIBagSearch",panel,"SearchBoxTemplate")
    ...
    local hideBoundCheck=CreateFrame("CheckButton","RogUIBagHideBound",panel,"UICheckButtonTemplate")
    hideBoundCheck:SetSize(24,24); hideBoundCheck:SetPoint("TOPLEFT",10,-32)
    local hideBoundText=_G["RogUIBagHideBoundText"]
    ...
    local hideWarboundCheck=CreateFrame("CheckButton","RogUIBagHideWarbound",panel,"UICheckButtonTemplate")
    hideWarboundCheck:SetSize(24,24); hideWarboundCheck:SetPoint("LEFT",hideBoundCheck,"RIGHT",64,0)
    local hideWarboundText=_G["RogUIBagHideWarboundText"]
    ...
    local showBoundOnlyCheck=CreateFrame("CheckButton","RogUIBagShowBoundOnly",panel,"UICheckButtonTemplate")
    showBoundOnlyCheck:SetSize(24,24); showBoundOnlyCheck:SetPoint("LEFT",hideWarboundCheck,"RIGHT",64,0)
    local showBoundOnlyText=_G["RogUIBagShowBoundOnlyText"]
    ...
    -- Allow items to be dropped onto empty space in the bag panel.
    ...
    canvas:SetScript("OnMouseDown", function(self, button)
        if button == "RightButton" and CursorHasItem() then
            ClearCursor()
        end
    end)
    ...
    -- Drop zone for completing stack splits
    local dropZone = CreateFrame("Button", "RogUIBagDropZone", panel)
    dropZone:SetSize(120, 22)
    dropZone:SetPoint("BOTTOM", panel, "BOTTOM", 0, 10)
    ...
    dzBg:SetAllPoints()
    dzBg:SetColorTexture(0.15, 0.35, 0.15, 0.7)

    local dzLabel = dropZone:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    dzLabel:SetAllPoints()
    dzLabel:SetText("|cFF88FF88⇩ drop here|r")

    local function DropZoneReceive()
        if not CursorHasItem() then return end
        for _, bagID in ipairs(GetAllBagIDs()) do
            local numSlots = C_Container.GetContainerNumSlots(bagID)
            for slotID = 1, numSlots do
    ...
    dropZone:SetScript("OnReceiveDrag", DropZoneReceive)
    dropZone:SetScript("OnClick", DropZoneReceive)
    dropZone:SetScript("OnEnter", function()
        dzBg:SetColorTexture(0.2, 0.5, 0.2, 0.9)
        if CursorHasItem() then dzLabel:SetText("|cFFAAFFAA⇩ release here|r") end
    end)
    ...
    table.insert(UISpecialFrames,"RogUIBagPanel")
end
...
-- so the RogUI panel opens instead of the Blizzard combined bag.
local function HookBagFunctions()
    ...
    -- Hide Blizzard bag frames immediately
    if ContainerFrameCombinedBags then 
        ContainerFrameCombinedBags:Hide()
        ContainerFrameCombinedBags:SetShown(false)
    end
...
local function RebuildSubBagRows()
    ...
    for i,entry in ipairs(sbs) do
        ...
        row.sb:SetScript("OnClick",function()
            ...
            if n=="" then print("|cFF00CCFF[RogUI Bags]|r Name cannot be empty."); return end
            cap.name=n; cap.search=s
            if panel and panel:IsShown() then LayoutPanel() end
            print("|cFF00CCFF[RogUI Bags]|r Saved: |cFFFFD700"..n.."|r")
        end)
...
local function BuildBagsTab()
    bagsTabFrame=CreateFrame("Frame","RogUIBagsSettingsFrame",UIParent)
    ...
    if rl then rl:SetText("Replace default bag with this panel  |cFFAAAAAA(B key / backpack button opens RogUI Bags)|r") end
    ...
    ab:SetScript("OnClick",function()
        ...
        if n=="" then print("|cFF00CCFF[RogUI Bags]|r Enter a section name."); return end
        if not DB() then return end
...
local function HookFrame(frame)
    ...
    frame.Show=function(self) self:Hide() OpenPanel() end
    local origSetShown=frame.SetShown
    frame.SetShown=function(self,shown) if shown then self:Hide() OpenPanel() else self:Hide() end end
end
...
local subBagsLoginFired = false
local function OnSubBagsEvent(event, ...)
    if event=="PLAYER_LOGIN" then
        if subBagsLoginFired then return end
        subBagsLoginFired = true
        DB()
        BuildPanel()
        BuildBagsTab()
        HookBagFunctions()
        if ContainerFrameMixin then
            ...
            local origShow=ContainerFrameMixin.Show
            ContainerFrameMixin.Show=function(self) self:Hide() OpenPanel() end
            local origSetShown=ContainerFrameMixin.SetShown
            ContainerFrameMixin.SetShown=function(self,shown) if shown then self:Hide() OpenPanel() else self:Hide() end end
        end
...
API.RegisterEvent("Bags", "PLAYER_LOGIN",           function(...) OnSubBagsEvent("PLAYER_LOGIN", ...) end)
API.RegisterEvent("Bags", "BAG_UPDATE",              function(...) OnSubBagsEvent("BAG_UPDATE", ...) end)
API.RegisterEvent("Bags", "BAG_UPDATE_DELAYED",      function(...) OnSubBagsEvent("BAG_UPDATE_DELAYED", ...) end)
API.RegisterEvent("Bags", "PLAYER_EQUIPMENT_CHANGED",function(...) OnSubBagsEvent("PLAYER_EQUIPMENT_CHANGED", ...) end)
API.RegisterEvent("Bags", "ITEM_LOCK_CHANGED",       function(...) OnSubBagsEvent("ITEM_LOCK_CHANGED", ...) end)
...
API.Debug("[RogUI] SubBags loaded.")