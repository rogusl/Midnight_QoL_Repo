-- ============================================================
-- MidnightQoL_SmartSwap / SmartSwap.lua
--
...
alsEvents:RegisterEvent("CHALLENGE_MODE_START")    -- M+ keystone start
alsEvents:RegisterEvent("GROUP_ROSTER_UPDATE")
...
alsEvents:RegisterEvent("TRAIT_CONFIG_UPDATED")  -- fires on loadout switch and spec change
alsEvents:RegisterEvent("PLAYER_FLAGS_CHANGED")  -- fires when War Mode is toggled
...
SlashCmdList["ALSSNIFF"] = function()
    if sniffActive then
        ...
        sniffActive = false
        print("|cFF00CCFF[MidnightQoL]|r ALS event sniffer |cFFAAAAAAdisabled|r")
    else
        if not API.DEBUG then
            API.DEBUG = true
            print("|cFF00CCFF[MidnightQoL]|r Debug mode auto-enabled for sniffer")
        end
        ...
        sniffActive = true
        print("|cFF00CCFF[MidnightQoL]|r ALS event sniffer |cFFFFFF00ON|r — swap a loadout or spec and watch chat")
    end
end
...
-- ── UI ────────────────────────────────────────────────────────────────────────
local alsFrame = CreateFrame("Frame", "MidnightQoLSSFrame", UIParent)
alsFrame:SetSize(780, 600); alsFrame:Hide()

-- Scroll frame so the tab content can grow
local alsScroll = CreateFrame("ScrollFrame", "MidnightQoLSSScroll", alsFrame, "UIPanelScrollFrameTemplate")
alsScroll:SetPoint("TOPLEFT", 0, 0); alsScroll:SetPoint("BOTTOMRIGHT", -20, 0)
local alsContent = CreateFrame("Frame", nil, alsScroll)
alsContent:SetWidth(760); alsScroll:SetScrollChild(alsContent)
...
local function RefreshPanel()
    ...
    -- Column x positions: spec name (140px) | 6 activity columns (100px each)
    -- Total: 140 + 6*100 = 740px — fits in the 760px content width
    local SPEC_COL_W = 140
    local ACOL_W     = 100
    ...
    -- Hide excess rows
    for i = loRowIdx+1, #sw.loRows do
        local row = sw.loRows[i]
        if row then row.nameLbl:Hide(); row.layBtn:Hide(); row.eqBtn:Hide() end
    end

    y = y - 8
        alsContent:SetHeight(math.abs(y) + 16)
end
...
API.Debug("[MidnightQoL] SmartSwap loaded.")
