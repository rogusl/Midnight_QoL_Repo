...
title:SetPoint("TOPLEFT", 20, -20); title:SetText("MidnightQoL Configuration")
...
local function RegisterTab(label, frame, onActivate, widthOverride, onDeactivate)
...
minimapBtn:SetScript("OnEnter",function(self)
    ...
    GameTooltip:AddLine("MidnightQoL",1,1,0)
    ...
    if event=="PLAYER_LOGIN" then
        if not BuffAlertDB then
            BuffAlertDB={
                ...
                buffDebuffAlertsEnabled=true,whisperIndicatorEnabled=true,
        ...
        if API._profilesFrameReady then
            RegisterTab("Profiles", API._profilesFrame, nil, 90)
        end
