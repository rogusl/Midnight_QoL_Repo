-- ============================================================
-- RogUI / Modules / Whisper / Whisper.lua
-- MIGRATED: Unified event system
-- Whisper event handling, unread indicator, sound triggers.
-- Settings are ACCOUNT-WIDE (stored at RogUIDB root, not per-spec).
-- ============================================================

local API = RogUIAPI

-- ── State (account-wide, loaded from RogUIDB root) ────────────────────────
local whisperList            = {}
...
API.SetWhisperIndicator     = function(v)
    ...
    if not v then
        local icon = _G["RogUIUnreadMailIcon"]
        if icon then icon:Hide() end
    end
    if RogUIDB then RogUIDB.whisperIndicatorEnabled = v end
end

-- ── Load / Save from RogUIDB root (account-wide) ─────────────────────────
local function LoadWhisperSettings()
    ...
    for _, v in ipairs(RogUIDB.whisperList or {}) do table.insert(whisperList, v) end
    whisperEnabled          = RogUIDB.whisperEnabled          or false
    ignoreOutgoingWhispers  = (RogUIDB.ignoreOutgoingWhispers ~= false)
    whisperIndicatorEnabled = (RogUIDB.whisperIndicatorEnabled == true)
    -- Sync UI if open
...
local function SaveWhisperSettings()
    if not RogUIDB then return end
    RogUIDB.whisperList            = whisperList
    RogUIDB.whisperEnabled         = whisperEnabled
    RogUIDB.ignoreOutgoingWhispers = ignoreOutgoingWhispers
    RogUIDB.whisperIndicatorEnabled= whisperIndicatorEnabled
...
-- ── Unread mail icon ───────────────────────────────────────────────────────────
local unreadMailIcon = CreateFrame("Frame", "RogUIUnreadMailIcon", UIParent)
...
unreadMailIcon:SetScript("OnDragStop", function(self)
    ...
    if RogUIDB then
        ...
        RogUIDB.unreadMailIconPos = {point="CENTER", x=math.floor(ix-cx+0.5), y=math.floor(iy-cy+0.5)}
    end
end)
...
-- Tooltip on hover
local tooltip = CreateFrame("Frame","RogUIUnreadTooltip",UIParent,"BackdropTemplate")
...
function UpdateUnreadMailIcon()
    ...
    if count > 0 then
        unreadMailIcon:Show()
        if RogUIDB and RogUIDB.unreadMailIconPos then
            unreadMailIcon:ClearAllPoints()
            unreadMailIcon:SetPoint("CENTER",UIParent,"CENTER",
                RogUIDB.unreadMailIconPos.x or 0, RogUIDB.unreadMailIconPos.y or 0)
        end
...
-- whisperEvents frame replaced by API.RegisterEvent calls
...
local function OnWhisperEvent(event, ...)
    if event == "PLAYER_LOGIN" then
        ...
        if RogUIDB and RogUIDB.generalWhisperSound then
            generalSoundDropdown:SetSelectedSound(RogUIDB.generalWhisperSound, RogUIDB.generalWhisperSoundIsID)
        end
...
API.RegisterEvent("Whisper", "PLAYER_LOGIN",                function(...) OnWhisperEvent("PLAYER_LOGIN", ...) end)
API.RegisterEvent("Whisper", "CHAT_MSG_WHISPER",            function(...) OnWhisperEvent("CHAT_MSG_WHISPER", ...) end)
API.RegisterEvent("Whisper", "CHAT_MSG_WHISPER_INFORM",     function(...) OnWhisperEvent("CHAT_MSG_WHISPER_INFORM", ...) end)
API.RegisterEvent("Whisper", "CHAT_MSG_BN_WHISPER",         function(...) OnWhisperEvent("CHAT_MSG_BN_WHISPER", ...) end)
API.RegisterEvent("Whisper", "CHAT_MSG_BN_WHISPER_INFORM",  function(...) OnWhisperEvent("CHAT_MSG_BN_WHISPER_INFORM", ...) end)

-- ── Layout handle provider for the unread icon ────────────────────────────────
API.RegisterLayoutHandles(function()
    local ox, oy = 0, 0
    if RogUIDB and RogUIDB.unreadMailIconPos then
        ox = RogUIDB.unreadMailIconPos.x or 0
        oy = RogUIDB.unreadMailIconPos.y or 0
    else
...
SlashCmdList["CUSTOMWHISPERTEST"] = function()
    ...
    UpdateUnreadMailIcon()
    print("|cFF00FF00[RogUI]|r Added test whisper. Count: "..#unreadWhispers)
end
